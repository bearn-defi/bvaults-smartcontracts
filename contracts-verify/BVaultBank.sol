...
contract BvaultsBank is ReentrancyGuard {
    ...
    function shutdownStrategy(uint256 _pid) public onlyOperator {
        poolInfo[_pid].strategy = address(0);
    }
    ...
    function setBlacklistAccount(address _account, bool _status) external nonReentrant onlyOperator {
        blacklistAccount[_account] = _status;
    }

    function setBlacklistAccounts(address[] memory _accounts, bool _status) external nonReentrant onlyOperator {
        uint256 _length = _accounts.length;
        for (uint256 i = 0; i < _length; i++) {
            blacklistAccount[_accounts[i]] = _status;
        }
    }

    // to rescue stuck fund in the exploited BUSD Alpaca Strategy
    function withdrawFundToCompensationContract(uint256 _wantAmt) external nonReentrant onlyOperator {
        IERC20 _busd = IERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
        uint256 _before = _busd.balanceOf(address(this));
        IStrategy(0x21125d94Cfe886e7179c8D2fE8c1EA8D57C73E0e).withdraw(address(this), _wantAmt); // exploited BUSD Alpaca Strategy
        uint256 _amount = _busd.balanceOf(address(this)).sub(_before);
        if (_amount > 0) {
            _busd.transfer(address(0xEfAFf6c4b1fab061914A5747bC8b13D97C9466B3), _wantAmt);
        }
    }
