...
contract BvaultsStrategy is Ownable, ReentrancyGuard, Pausable {
    ...
    address public bdobusdLPAddress;
    ...
    address public vPegSwapRouterAddress = address(0xC437B8D65EcdD43Cda92739E09ebd68BBE1965e1);
    address public vSwapPool = address(0x18A78291a4A903ee5e8d8CE5a5945ecd2388C0f8);
    address public vSwapBasePool = address(0x7569f9adabC99780B7A91B16666Bb985177D1DCa);
    ...
    address public constant bdoVDollarAddress = address(0x33979E0814Dc36942D92F5807F08a2819a38fd66);
    ...
    constructor(
        ...
        uint256 _pid,
        address _bdobusdLPAddress,
        address _earnedAddress,
        address _uniRouterAddress,
        address _vPegSwapRouterAddress,
        ...
        bdobusdLPAddress = _bdobusdLPAddress;

        if (_uniRouterAddress != address(0)) uniRouterAddress = _uniRouterAddress;
        if (_vPegSwapRouterAddress != address(0)) vPegSwapRouterAddress = _vPegSwapRouterAddress;
    ...
    function migrate() public  returns (uint256) {
        // transfer vdollar to strategy
        uint256 amt = IERC20(bdobusdLPAddress).balanceOf(address(msg.sender));
        IERC20(bdobusdLPAddress).safeTransferFrom(address(msg.sender), address(this), amt);
        // add vdollar to vpegswap to get bdoVdollar LP
        IERC20(bdobusdLPAddress).safeIncreaseAllowance(vPegSwapRouterAddress, _wantAmt);
        uint256[] memory meta_amounts = new uint256[](2);
        uint256[] memory base_amounts = new uint256[](4);
        meta_amounts[1] = _wantAmt;

        IStableSwapRouter(vPegSwapRouterAddress).addLiquidity(vSwapPool, vSwapBasePool, meta_amounts, base_amounts, 1, block.timestamp);

        uint256 amt = IERC20(bdoVDollarAddress).balanceOf(address(this));

        uint256 sharesAdded = amt;
        if (wantLockedTotal > 0 && sharesTotal > 0) {
            sharesAdded = amt.mul(sharesTotal).mul(entranceFeeFactor).div(wantLockedTotal).div(entranceFeeFactorMax);
        }
        ...
        if (isAutoComp) {
            ...
            wantLockedTotal = wantLockedTotal.add(amt);
        }
    ...
    function _farm() internal {
        uint256 wantAmt = IERC20(bdobusdLPAddress).balanceOf(address(this));
        wantLockedTotal = wantLockedTotal.add(wantAmt);
        IERC20(bdobusdLPAddress).safeIncreaseAllowance(farmContractAddress, wantAmt);
    ...
    function withdraw(address, uint256 _wantAmt) public onlyOwner nonReentrant returns (uint256) {
        ...
        IERC20(bdobusdLPAddress).safeTransfer(address(msg.sender), _wantAmt);
    ...
    function earn() public whenNotPaused {
        ...
        if (earnedAddress != token0Address) {
            ...
            IPancakeRouter02(uniRouterAddress).swapExactTokensForTokensSupportingFeeOnTransferTokens(earnedAmt, 0, paths[earnedAddress][token0Address], address(this), now + 60);
        }

        // Get want tokens, ie. add liquidity
        uint256 token0Amt = IERC20(token0Address).balanceOf(address(this));
        if (token0Amt > 0) {
            IERC20(token0Address).safeIncreaseAllowance(vPegSwapRouterAddress, token0Amt);
                uint256[] memory meta_amounts = new uint256[](2);
                uint256[] memory base_amounts = new uint256[](4);
                base_amounts[3] = token0Amt;

            IStableSwapRouter(vPegSwapRouterAddress).addLiquidity(vSwapPool, vSwapBasePool, meta_amounts, base_amounts, 1, block.timestamp);
            emit Compound(token0Address, token0Amt, address(0), 0)
        }
    ...
    function setVPegSwapInnfo(
        address _vSwapPool,
        address _vSwapBasePool
    ) external onlyOperator {
        vSwapPool = _vSwapPool;
        vSwapBasePool = _vSwapBasePool;
    }
    ...
    function inCaseTokensGetStuck(address _token, uint256 _amount, address _to) external onlyOperator {
        ...
        require(_token != bdobusdLPAddress, "!safe");
